...
class SQLiteHelper (context:Context):SQLiteOpenHelper(context, DATABASE_NAME,null,DATABASE_VERSION) {
    ...
    companion object {
        ...
        private const val resourcesID = "resourcesID"
        private const val name = "name"
        private const val email = "email"
        private const val resourceTitle = "resourceTitle"
        private const val resourceDocument = "resourceDocument"
        private const val resourceDesc = "resourceDesc"
        private const val savedEmail = "savedEmail"

    }


    override fun onCreate(db: SQLiteDatabase?) {
        val createTableResources = ("CREATE TABLE IF NOT EXISTS $TABLE_RESOURCES ("
            + "$resourcesID TEXT PRIMARY KEY,"
            + "$name TEXT,"
            + "$email TEXT,"
            + "$resourceTitle TEXT,"
    ...
    fun insertResources(resource: ResourceModel): Boolean {
        ...
        contentValues.put("resourcesID", resource.resourceId)
        ...
        contentValues.put("savedEmail", resource.savedEmail)
        val result = db.insert(TABLE_RESOURCES, null, contentValues)
        ...
        try {
            if (cursor.moveToFirst()) {
                resource = ResourceModel(
                    ...
                    resourceDesc = cursor.getString(cursor.getColumnIndexOrThrow("resourceDesc")),
                    savedEmail = cursor.getString(cursor.getColumnIndexOrThrow("savedEmail"))
                )
            } else {
